...
| T-003 | todo | Copy source repo into app package | @you | `rsync -av --exclude='.git' --exclude='node_modules' --exclude='dist' --exclude='coverage' --exclude='release' ../tictactoe/ apps/tictactoe/` | `apps/tictactoe/package.json` exists |
| T-004 | todo | Set package identity | @you | edit `apps/tictactoe/package.json` name → `@games/tictactoe` | `pnpm --filter @games/tictactoe` resolves |
| T-005 | todo | Add missing script aliases | @you | add `web:build`, `cap:build:android`, `cap:build:ios` | scripts present in package.json |
...
| T-008 | todo | Validate core contract | @you | `pnpm --filter @games/tictactoe lint && pnpm --filter @games/tictactoe typecheck && pnpm --filter @games/tictactoe test && pnpm --filter @games/tictactoe validate` | all pass |
...
- [ ] T-003 Copy source repo into `apps/tictactoe`
- [ ] T-004 Set package identity to `@games/tictactoe`
- [ ] T-005 Add missing script aliases
